...
Modified Jan 2015 by Paul W. Rogers to work with neomatrix panel on pin 6, 
and fix a few compile bugs with arduino 1.5.7
...
// PWR Jan 2015
// needs #define __PROG_TYPES_COMPAT__ due to ffft library data type

#define __PROG_TYPES_COMPAT__
#include <avr/pgmspace.h>
//#define FFT_N 64  // can define FFT_N before include of ffft.h if not defined locally it is defined as 128
#include <ffft.h>
#include <math.h>
#include <Wire.h>
...
const PROGMEM uint8_t
  ...
  // PWR Jan 2015
  // mod col0 as low two bins are high noise and odd characteristics (can decrease in value on signal)
  // originally 2,1,111,8
  // may be an overflow bug in code somewhere as well
  col0data[] = {  2,  1,  // # of spectrum bins to merge, index of first
    2,   8 },           // Weights for each bin
  col1data[] = {  4,  1,  // 4 bins, starting at index 1
     1, 186,  38,   2 }, // Weights for 4 bins.  Got it now?
  col2data[] = {  5,  2,
     11, 156, 118,  16,   1 },
  col3data[] = {  8,  3,
  ...
  // PWR Jan 2015
  // had problems making this progmem so made this a normal memory array.
const uint8_t *colData[] = {
    col0data, col1data, col2data, col3data,
    col4data, col5data, col6data, col7data };

// PWR Jan 2015
// use neomatrix panel instead
...
void setup() {
  ...
  for(i=0; i<8; i++) {
    ...
    maxLvlAvg[i] = 512;
    // PWR Jan 2015
    // as this is no longer in progmem dont need to do the pgm_read bit....
    // data   = (uint8_t *)pgm_read_word(&colData[i]);
    data         = (uint8_t *) colData[i];
    nBins        = pgm_read_byte(&data[0]) + 2;  // +2 offsets the counter to data values
    binNum       = pgm_read_byte(&data[1]);
    for(colDiv[i]=0, j=2; j<nBins; j++)
      colDiv[i] += pgm_read_byte(&data[j]);
  }
  // PWR Jan 2015
  // neomatrix setup instead
  //matrix.begin(0x70);
  matrix.begin();
...
void loop() {
  ...
  // PWR Jan 2015
  // debug output fft results - beware this will slow down responsiveness of panel if using serial port output
/*
  for (int bin=0; bin<FFT_N/2; bin++)
  { Serial.print(spectrum[bin]);
    Serial.print(",");
  }
  Serial.println();  
*/
  ...
  for(x=0; x<8; x++) {
    ...
    binNum = pgm_read_byte(&data[1]);
    // PWR 2015 test the data is retreived ok
    /*
    Serial.print(x);
    Serial.print(",");
    Serial.print(nBins);
